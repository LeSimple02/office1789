...
(function() {
    ...
    // Détection environnement: prod (office1789.com) vs local
    ...
    // SI TOKEN SSO PRESENT, TRAITER EN PRIORITE (ne pas bloquer)
    if (ssoToken) {
        console.log('[Office1789-SSO] Token SSO détecté, traitement prioritaire...');
        // Le reste du script s'occupera du SSO
    } else if (isLoginPage() && !isAuthenticated) {
        // PAS de token SSO ET PAS authentifié => BLOQUER
        console.warn('[Office1789-SSO] Accès à la page de login bloqué');
    ...
    async function checkSSOToken() {
        if (!ssoToken) {
            console.log('[Office1789-SSO] Pas de token SSO');
            return;
        }
            ...
            document.body.innerHTML = `
                <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;font-family:Arial,sans-serif;background:#f5f5f7;color:#333;">
                    <div style="text-align:center;background:white;padding:48px;border-radius:24px;box-shadow:0 8px 32px rgba(0,0,0,0.1);">
                        ...
                        <a href="${FRONT_BASE}" style="display:inline-block;padding:12px 32px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;text-decoration:none;border-radius:24px;font-weight:600;">
                            Retour à Office1789
                        </a>
                    </div>
                </div>
