...
if (empty($sso_token)) {
    ...
    if (!$is_static_resource && !$is_post_sso && !$is_authenticated_task) {
        ...
        echo '<!DOCTYPE html><html><head><title>Accès refusé</title><style>body { font-family: Arial; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); } .box { background: white; padding: 40px; border-radius: 10px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); text-align: center; max-width: 500px; } h1 { color: #e74c3c; margin: 0 0 20px 0; } p { color: #555; line-height: 1.6; } a { display: inline-block; margin-top: 20px; padding: 12px 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; border-radius: 5px; }</style></head><body><div class="box"><h1>🔒 Accès Refusé</h1><p>L\'accès direct à Roundcube est désactivé.</p><p>Veuillez vous connecter via <strong>Office1789</strong>.</p><a href="http://localhost:5173">Retour à Office1789</a></div></body></html>';
        exit;
    }
}
