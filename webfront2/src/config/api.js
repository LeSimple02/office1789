export const API_CONFIG = {
  ...
  getBaseURL() {
    // 0. Runtime config generated by nginx entrypoint (app-config.js)
    try {
      if (window && window.__APP_CONFIG__ && window.__APP_CONFIG__.VITE_APP_API) {
        if (import.meta.env.DEV) console.log('[API] Using runtime app-config VITE_APP_API:', window.__APP_CONFIG__.VITE_APP_API)
        return window.__APP_CONFIG__.VITE_APP_API
      }
    } catch(e) {
    ...
    if (import.meta.env.VITE_APP_API) {
      if (import.meta.env.DEV) console.log('[API] Using VITE_APP_API:', import.meta.env.VITE_APP_API)
    ...
    if (protocol === 'file:' || window.cordova) {
      return import.meta.env.VITE_APP_API || localStorage.getItem('api_url') || localStorage.getItem('auto_api_url') || 'http://192.168.56.1:8080'
    }

    // Electron (app://)
    if (window.electron) {
      return import.meta.env.VITE_APP_API || localStorage.getItem('api_url') || 'http://localhost:8080'
    }
    
    // Web development local
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
      return import.meta.env.VITE_APP_API || 'http://localhost:8080'
    }

    // Production - si on est sur office1789.com
    if (hostname.includes('office1789.com')) {
      // In production prefer the build-time env var or runtime config
      return import.meta.env.VITE_APP_API || 'https://backend.office1789.com'
    }

    // Fallback production: prefer env if present, otherwise backend.office1789
    return import.meta.env.VITE_APP_API || 'https://backend.office1789.com'
  },
