    ...
    <section v-else class="card">
      ...
      <form class="form-grid" @submit.prevent="send" novalidate>
        ...
        <div class="form-group">
          ...
          <!-- Le nom d'utilisateur est maintenant immuable : affiché en lecture seule -->
          <input :value="gls().username" class="input" disabled />
        ...
        <div class="form-group">
          ...
          <div v-if="newemail && emailValid" class="verification-actions">
            <button 
              type="button" 
              class="btn-secondary verify-btn" 
            ...
            <p v-if="emailVerificationSent" class="success-msg">
              ✅ {{ $t('verificationCodeSent') || 'Code envoyé, vérifiez votre email.' }}
            </p>
          </div>

          <div v-if="newemail" class="verification-code">
            <input 
              v-model="emailCode" 
              class="input" 
            ...
            <p v-if="emailVerificationSuccess" class="success-msg">
              ✅ {{ $t('verificationCodeValid') || 'Code validé, email confirmé.' }}
            </p>
            <p v-if="emailVerificationError" class="error-msg">{{ emailVerificationError }}</p>
          </div>
          <p v-if="newemail && !emailValid" class="error">❌ Format d'email invalide</p>
          <p v-if="emailR" class="error">{{ $t('dejaEP') }}</p>
        </div>

        <!-- Phone (désactivé pour le moment) -->
      ...
      <div class="subscription-zone">
        ...
        <div class="current-plan">
          ...
          <p class="plan-details">
            ...
            <span v-if="plans[nboffer]"> - {{ nboffer === 3 ? '100 To' : plans[nboffer].storage }} de stockage</span>
          </p>
        </div>
        ...
        <div class="plans-grid">
          <div v-for="plan in plans" :key="plan.id" class="plan-card" :class="{ 'current': plan.id === nboffer, 'recommended': plan.id === 2 }">
            ...
            <p class="plan-storage">{{ plan.id === 3 ? '100 To' : plan.storage }} de stockage</p>
        ...
        <div v-if="nboffer >= 2" class="organization-link-section">
          ...
          <RouterLink to="/account/custom-domain" class="organization-link-btn">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <circle cx="12" cy="12" r="10"></circle>
            ...
            <div class="link-content">
              <h4>🌐 Domaine personnalisé</h4>
              <p>Utilisez votre propre domaine pour vos emails (ex: contact@votreentreprise.com)</p>
            </div>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <polyline points="9 18 15 12 9 6"></polyline>
            </svg>
        ...
        <div class="modal subscription-modal" @click.stop>
          ...
          <div class="modal-body" v-if="plans[selectedPlan]">
            <div class="selected-plan-summary">
              ...
              <p class="storage-info">{{ selectedPlan === 3 ? '100 To' : plans[selectedPlan].storage }} de stockage</p>
            </div>
