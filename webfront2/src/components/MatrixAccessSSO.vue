...
const openMatrix = async () => {
  ...
  if (isLoadingMatrix.value) {
    console.log('[Matrix SSO] Requête déjà en cours, ignorée')
    return
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Toujours privilégier l'URL retournée par le backend (avec sso_token)
      // puis retomber sur les URLs de configuration si nécessaire.
      const preferred =
        (data.url && data.url.trim()) ||
        (import.meta.env.VITE_ELEMENT_URL && import.meta.env.VITE_ELEMENT_URL.trim()) ||
        (import.meta.env.VITE_MATRIX_URL && import.meta.env.VITE_MATRIX_URL.trim())
