...
func createMailAccount(username, password string) error {
	...
	err := cmd.Run()
	if err != nil {
		// Si le compte existe dǸj�� dans le mailserver, on met simplement �� jour son mot de passe
		if strings.Contains(stderr.String(), "already exists") {
			fmt.Printf("Info: mail account %s already exists, updating password instead of creating\n", email)
			if errChange := changeMailPassword(username, password); errChange != nil {
				return fmt.Errorf("failed to update existing mail account password: %v", errChange)
			}
			return nil
		}
