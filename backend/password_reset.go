...
func RequestPasswordReset(c *gin.Context) {
	...
	if frontendURL == "" {
		if domain := os.Getenv("DOMAIN_BASE"); domain != "" {
...
func ResetPassword(c *gin.Context) {
	...
	// Synchroniser immediatement le mot de passe avec Mail (Docker Mailserver)
	go func() {
		if err := changeMailPassword(username, req.NewPassword); err != nil {
			fmt.Printf("Warning: Failed to change mail password for %s during reset: %v\n", username, err)
		} else {
			fmt.Printf("Mail password changed for %s@office1789.com via password reset\n", username)
		}
	}()

	// Synchroniser immediatement le mot de passe avec Matrix (Synapse)
	go func() {
		if err := changeMatrixPassword(username, req.NewPassword); err != nil {
			fmt.Printf("Warning: Failed to change matrix password for %s during reset: %v\n", username, err)
		} else {
			fmt.Printf("Matrix password changed for @%s:office1789.com via password reset\n", username)
		}
	}()
...
func sendResetEmail(to, username, resetLink string) error {
	...
	var auth smtp.Auth
	if smtpUser != "" && smtpPass != "" {
		// Autoriser l'auth même si STARTTLS n'est pas annoncé (réseau interne)
...
`, username, resetLink, resetLink)
	...
	// Envoyer l'email (avec authentification si configurée)
	...
	if err != nil && (strings.Contains(err.Error(), "doesn't support AUTH") || strings.Contains(err.Error(), "unencrypted")) {
		// Serveur SMTP sans AUTH/STARTTLS (dev) : retenter sans authentification
