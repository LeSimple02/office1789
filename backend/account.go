...
func ChangeI(c *gin.Context) {
	...
	// Username IMMUTABLE après création : toute tentative de changement est bloquée
	if strings.TrimSpace(cha.Username) != "" && cha.Username != cha.LastUsername {
		fmt.Println("DEBUG ChangeI - Username change attempt blocked (username is immutable)")
		infova.Username = "locked"
